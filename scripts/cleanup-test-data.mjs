...
//   SUPABASE_URL=... SUPABASE_SERVICE_ROLE_KEY=... node scripts/cleanup-test-data.mjs        # dry run (lists only)
//   SUPABASE_URL=... SUPABASE_SERVICE_ROLE_KEY=... node scripts/cleanup-test-data.mjs --yes   # actually delete
//
...
if (!url || !serviceKey) {
    console.error('Missing env. Set SUPABASE_URL and SUPABASE_SERVICE_ROLE_KEY (service role, not the anon key).');
    process.exit(1);
}
