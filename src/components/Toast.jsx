...
// Hook returning showToast(message, type) — the canonical way screens trigger a toast.
// `type` is optional: 'success' | 'error' | undefined (neutral).
...
export const ToastProvider = ({ children }) => {
    ...
    return (
        <ToastContext.Provider value={showToast}>
            {children}
            {toast && (
                <div className={`toast${toast.type ? ` toast-${toast.type}` : ''}`} role="status" key={toast.id}>
                    {toast.message}
                </div>
            )}
